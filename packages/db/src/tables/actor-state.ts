...
export async function upsertActorStateRow(
  db: DrizzleClient,
...
export async function listAllActorStates(db: DrizzleClient) {
  return db.select().from(actorState);
}

export async function getActorStateById(
  db: DrizzleClient,
  id: string
): Promise<ActorStateRow | null> {
...
export async function countAllActorStates(db: DrizzleClient) {
  const result = await db.select({ c: count() }).from(actorState);
