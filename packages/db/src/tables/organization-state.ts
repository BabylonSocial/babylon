...
export async function upsertOrganizationStateRow(
  db: DrizzleClient,
  id: string,
...
export async function getOrganizationStateById(
  db: DrizzleClient,
  id: string
...
export async function listAllOrganizationStates(db: DrizzleClient) {
  return db.select().from(organizationState);
}

export async function listOrganizationStatesByPriceDesc(db: DrizzleClient) {
  return db
    .select()
    .from(organizationState)
...
export async function countAllOrganizationStates(db: DrizzleClient) {
  const result = await db.select({ c: count() }).from(organizationState);
