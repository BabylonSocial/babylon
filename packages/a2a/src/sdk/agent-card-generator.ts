...
import { fetchA2aAgentCardUserRow, fetchA2aAgentConfigRow } from '@babylon/db';
...
export async function generateAgentCard(agentId: string): Promise<AgentCard> {
  const user = await fetchA2aAgentCardUserRow(agentId);

  if (!user || !user.isAgent) {
    throw new Error(`Agent ${agentId} not found`);
  }

  const agentConfig = await fetchA2aAgentConfigRow(agentId);
