...
export const miscPaths: ZodOpenApiPathsObject = {
  ...
  '/api/upload/image': {
    ...
    delete: {
      ...
      description: 'Deletes a previously uploaded image by URL.',
      security: [{ PrivyAuth: [] }],
      requestBody: {
        required: true,
        content: {
  ...
  '/api/auth/farcaster/callback': {
    post: {
      ...
      responses: {
        '200': {
          description: 'Farcaster linked successfully',
          content: {
            'application/json': {
              schema: z
                .object({
                ...
                .meta({ id: 'FarcasterCallbackResponse' }),
            },
          },
        },
        '400': { description: 'Invalid request' },
