Getting Started / IntroductionYour First RouteHandler and ContextStatus and HeadersValidationLifecycleGuardPluginEncapsulationCookieError HandlingValidation ErrorExtends ContextStandalone SchemaMacroOpenAPIMountUnit TestEnd-to-End Type SafetyWhat&#39;s Next?Documentation# Status [​](#status)
...
See [Status](#/essential/handler#status).
...
See [Redirect](#/essential/handler#redirect).
...
See [Headers](#/essential/handler#set-headers).

## Assignment [​](#assignment)

Let's exercise what we have learned.

Show answer- To set status code to `418 I'm a teapot`, we can use `status` function.
- To redirect `/docs` to `https://elysiajs.com`, we can use `redirect` function.
- To set a custom header `x-powered-by` to `Elysia`, we can use `set.headers` object.
typescript```
import { Elysia } from 'elysia'
