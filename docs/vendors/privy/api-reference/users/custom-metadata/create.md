...
Learn more about custom metadata using our SDKs [here](/user-management/users/custom-metadata).


## OpenAPI

````yaml post /v1/users/{user_id}/custom_metadata
openapi: 3.1.0
info:
  version: 0.0.1
  title: Privy API
...
paths:
  /v1/users/{user_id}/custom_metadata:
    post:
      tags:
        - Users
      summary: Create Custom Metadata
      description: Adds custom metadata to a user by user ID.
      parameters:
        - schema:
            type: string
      ...
      responses:
        '200':
          description: User with updated custom metadata.
          content:
            application/json:
              schema:
